...
- [x] создать `core/audio/.../source/PcmAudioSource.kt` — `interface PcmAudioSource { val sampleRateHz: Int; fun samples(): Flow<ShortArray>; suspend fun isUnprocessedSupported(): Boolean }` (Flow закрывается при отмене корутины; внутри Flow вызывает `AudioRecord.start()` в `flow { ... }` с `try-finally` для `record.stop(); record.release()`)
- [x] создать `source/AudioRecordPcmSource.kt` — `class AudioRecordPcmSource @Inject internal constructor(@ApplicationContext private val context: Context, private val sessionFactory: AudioRecordSessionFactory, @IoDispatcher private val ioDispatcher: CoroutineDispatcher) : PcmAudioSource` — конфигурация по § 10 спеки: `sampleRateHz = 48_000` (с fallback на 44.1 кГц по `getMinBufferSize`); `AudioSource.UNPROCESSED` (API 24+) с fallback на `VOICE_RECOGNITION` далее на `MIC`; bufferSize `max(getMinBufferSize, sampleRate/10 * 2)` (≥ 100 ms); read через `AudioRecord.read(ShortArray, 0, len)`; в `flow { ... }` цикл `while (currentCoroutineContext().isActive)`. Вынесен внутренний `AudioRecordSessionFactory` (production `AndroidAudioRecordSessionFactory`, test fakes) — упрощает unit-тесты без Robolectric.
- [x] создать `source/AudioRecordCheck.kt` — utility `fun isUnprocessedSupported(context: Context): Boolean` через `AudioManager.getProperty(AudioManager.PROPERTY_SUPPORT_AUDIO_SOURCE_UNPROCESSED) == "true"`; добавлена перегрузка `isUnprocessedSupported(audioManager: AudioManager)` для прямого MockK-тестирования без Robolectric (humble-object pattern).
- [x] добавить в `AndroidManifest.xml` `:core:audio` `<uses-permission android:name="android.permission.RECORD_AUDIO" tools:node="merge"/>` (permission уже объявлено в `:app`-манифесте, но добавляем в audio-модуль для лучшей discoverability)
- [x] **сначала тест:** `AudioRecordPcmSourceShadowTest` (`@RunWith(RobolectricTestRunner)`) — переименован в "smoke-only" Robolectric тест: проверяет что `AndroidAudioRecordSessionFactory` корректно работает с реальным `AudioRecord` API под Robolectric (positive min buffer для 48 kHz, успешная аллокация MIC-сессии, корректное определение sampleRateHz). Детальная *политика* fallback вынесена в `AudioRecordPcmSourceTest` (быстрые JVM-тесты с `FakeAudioRecordSessionFactory`), потому что Robolectric ShadowAudioRecord не позволяет симулировать `IllegalArgumentException` в конструкторе или менять `getMinBufferSize` результат.
- [x] **сначала тест:** `AudioRecordPcmSourceTest` (JVM, JUnit 5 + MockK + Turbine) — 17 тестов покрывают: приоритет UNPROCESSED при `getProperty == "true"`; скип UNPROCESSED при `getProperty == "false"`; fallback UNPROCESSED → VOICE_RECOGNITION → MIC; sample rate fallback 48k → 44.1k; throw когда нет ни одного поддерживаемого rate; ≥ 100 ms buffer; правильную эмиссию PCM-чанков; lifecycle (start/stop/release) при flow completion и take-cancellation; fallback на VOICE_RECOGNITION когда `AudioManager == null`; companion-константы.
- [x] **сначала тест:** `AudioRecordCheckTest` (JUnit 5 + MockK, без Robolectric) — параметризованный `@CsvSource` на 5 значений property ("true"/"false"/"TRUE"/null/""), плюс тест с реальным mocked Context, плюс тест когда AudioManager service отсутствует.
- [x] реализовать классы чтобы тесты позеленели
- [x] добавить Hilt-модуль `core/audio/.../di/AudioModule.kt` с `@Binds` для `PcmAudioSource → AudioRecordPcmSource`, `@Binds` для `AudioRecordSessionFactory → AndroidAudioRecordSessionFactory`, `@Provides` для `AudioProcessorFactory` и `@IoDispatcher`-qualified `Dispatchers.IO`. Реализован как Kotlin `interface` (а не `abstract class`) — детект `UnnecessaryAbstractClass` предпочитает interface когда все методы абстрактны; Hilt поддерживает interface-modules с companion object для `@Provides`.
- [x] run `./gradlew :core:audio:testDebugUnitTest` — must pass before next task (✅ 113 тестов в `:core:audio` зелёные, включая 27 новых в `source/`)
...
- [x] создать `feature/measure/.../ui/SplReadout.kt` — `@Composable fun SplReadout(db: Float, modifier: Modifier)`: 96 sp tabular numeral (font-feature `tnum` чтобы цифры не прыгали на 10 Hz refresh) + "дБ A" suffix; цвет берётся из `levelToSplColor(db, palette)`; вся строка получает единый `contentDescription` через `Modifier.semantics(mergeDescendants = true)` чтобы TalkBack произносил "63 децибел по шкале A" одной фразой
- [x] создать `ui/SplArcGauge.kt` — `@Composable fun SplArcGauge(db: Float, modifier: Modifier)`: 270° дуга через `Canvas`, sweep-gradient из 6 цветов SPL-палитры; индикатор-стрелка вычисляет угол через линейную интерполяцию `db` в окне [30..110]; реф-метка через `dbToReferenceLabel(db: Float): @StringRes Int` с таблицей `ReferenceBuckets: List<Pair<Float, Int>>` (13 ступеней из спеки § 6, от "Дыхание" до "Реактивный самолёт"); non-finite вход даёт `measure_ref_breath` (как `levelToSplColor` falls back to veryQuiet)
- [x] создать `ui/SplLineChart.kt` — `@Composable fun SplLineChart(samples: List<SoundSample>, modifier: Modifier)`: `Canvas` + `Path`, точки нормализуются по окну `maxOf(60_000, latest-earliest)` ms (чтобы свежий буфер не сжимался к правому краю) и Y-диапазону 30..110 dB; цвет = `levelToSplColor(last.db, palette)`; меньше 2 семплов = ничего не рисуется (защита от вырожденной линии)
- [x] создать `ui/SplStatsRow.kt` — три Material 3 `Card(filled)` равной ширины с лейблами "Мин/Сред/Макс" и значениями `headlineSmall`; sentinel-значения (`Float.POSITIVE_INFINITY` / `Float.NEGATIVE_INFINITY`) рендерятся как `—` через `measure_stats_placeholder`
- [x] создать `ui/MeasureBottomBar.kt` — Reset IconButton слева (`enabled` только в `Paused`), Start/Pause FAB по центру (PlayArrow ↔ Pause в зависимости от phase), Save IconButton справа (`enabled = false` в Phase 2 + dedicated content description `measure_save_disabled_cd`); navigation-bar inset через `windowInsetsPadding(WindowInsets.navigationBars)`
- [x] создать `ui/PermissionRationaleDialog.kt` — Material 3 `AlertDialog` с двумя кнопками (Разрешить / Не сейчас); текст из FR-2 явно гарантирует "Аудио НЕ записывается и НЕ покидает устройство"
- [x] переписать `MeasureScreen.kt`: внутренний `MeasureScreenContent(state, onEvent, snackbarHostState, showRationale, onRationaleConfirm, onRationaleDismiss)` для unit-тестабельности + публичный `MeasureScreen(viewModel: MeasureViewModel = hiltViewModel())` который связывает permission launcher (`rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission())` + `ActivityCompat.shouldShowRequestPermissionRationale` для отличия Denied от PermanentlyDenied) и обработчик `MeasureUiEffect` (RequestPermission/ShowSnackbar/OpenAppSettings). Сознательное отклонение от плана: `Scaffold` без LargeTopAppBar — app-level `TishinaApp` уже хостит `CenterAlignedTopAppBar` через `TishinaApp.kt`, дублирование породило бы две панели поверх друг друга. Сигнатура `onNavigateToAbout: () -> Unit` не нужна — навигация About уже хостится в app-уровне.
- [x] **сначала тест:** `SplReadoutScreenshotTest` — 8 baseline (0.0 / 45.5 / 78.3 / 105.7 dB × light/dark)
- [x] **сначала тест:** `SplArcGaugeScreenshotTest` — 8 baseline (30 / 60 / 85 / 110 dB × light/dark)
- [x] **сначала тест:** `SplLineChartScreenshotTest` — 2 baseline (синусоида 40..80 dB, light/dark)
- [x] **сначала тест:** `SplStatsRowScreenshotTest` — 4 baseline (заполненная строка min=40/avg=65/max=90 и empty-state с sentinel-значениями, ×light/dark)
- [x] **сначала тест:** `MeasureBottomBarScreenshotTest` — 6 baseline (Idle/Running/Paused × light/dark)
- [x] **сначала тест:** `PermissionRationaleDialogScreenshotTest` — 2 baseline; использует `onAllNodes(isRoot()).onLast()` чтобы выбрать окно-overlay диалога (Material 3 `AlertDialog` создаёт второй Compose-root, и `onRoot()` упал бы с "Expected 1 node but found 2")
- [x] **сначала тест:** `MeasureScreenIdleScreenshotTest` — 2 baseline (Idle + permissionState=Unknown × light/dark); рендерит `MeasureScreenContent` напрямую с hand-built state, без Hilt и Activity
- [x] **сначала тест:** `MeasureScreenRunningScreenshotTest` — 2 baseline (Running, синусоидальная история 60 семплов, current=65, min=50/avg=62.4/max=75, duration=1m23s × light/dark)
- [x] **сначала тест:** `MeasureScreenComposeBehaviorTest` — 4 теста через `createComposeRule()`: FAB clickable + Save disabled в Idle; rationale dialog рендерится при `showRationale=true`; FAB tap в Idle → `StartRequested`; FAB tap в Running → `PauseRequested`. Третий пункт плана ("после `PermissionResult(granted=true)` + emit `60f` через FakeAudioRepository — `onNodeWithText("60.0")` присутствует") покрыт `MeasureViewModelStartTest` на ViewModel-уровне; в UI-тесте дублирование не нужно — содержимое экрана уже зафиксировано screenshot-тестами.
- [x] добавлен `DbToReferenceLabelTest` (JUnit 5 `@TestFactory`) — 15 reference-точек + NaN/±Infinity граница; `@CsvSource` использовать не получилось т.к. Android R.string не compile-time constants
- [x] инфра: `roborazzi` plugin + `testOptions.unitTests.isIncludeAndroidResources = true` + `material-icons-extended` + `activity-compose` + `lifecycle-runtime-compose` в `feature/measure/build.gradle.kts`; `robolectric.properties` с `sdk=33` в `src/test/resources/`
- [x] реализовать композблы; запущен `recordRoborazziDebug` — 34 baseline PNG записаны и визуально проверены (idle/running выглядят как ожидалось — 96 sp readout, цвет соответствует SPL-палитре, gauge needle поворачивается правильно, chart рисует синусоиду, bottom bar показывает Reset/FAB/Save)
- [x] run `./gradlew :feature:measure:testDebugUnitTest verifyRoborazziDebug` — ✅ 70 тестов зелёных, verifyRoborazziDebug проходит; `:app:assembleDebug` тоже зелёный (Hilt-граф резолвится через MeasureUseCaseModule + AudioModule); detektAll + spotlessCheck + lintDebug чистые
